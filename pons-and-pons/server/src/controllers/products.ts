...
const getAllSugarProducts = (req: Request, res: Response) => {
    const sugarProducts: Product[] = products.filter(p => p.category === 'sugar');
    res.status(200).json(sugarProducts);
}

const getAllSugarFreeProducts = (req: Request, res: Response) => {
    const sugarFreeProducts: Product[] = products.filter(p => p.category === 'sugar-free');
